...
class PostgresCollector(BaseDBCollector):
    ...
    def read_by_chunks_id(self):
        ...
        for event_date in date_ranges:
            ...
            if min_id is None or max_id is None:
                self.logger.warning(
                    f"No data for {'whole table' if not event_date else f'date {date_str}'}. Skipping."
                )
                continue

            est_num_partition = max(
                1, int((max_id - min_id + 1) // self.chunk_size) + 1
            )
            num_partition = min(est_num_partition, self.max_partition)
            self.logger.info(
                f"Total rows: {max_id - min_id}, numPartitions: {num_partition}"
            )
            ...
            output_file = (
                self._get_output_path(event_date)
                if event_date
                else f"{self.output_path}/all"
            )
    ...
    def read_full(self):
        ...
        for event_date in date_ranges:
            ...
            where_clause = self._build_where_clause(date_str)
            base_query = (
                f"SELECT {self.selected_columns} FROM {self.table_name} {where_clause}"
            )
