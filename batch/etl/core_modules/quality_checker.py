...
class QualityChecker(BaseProcessor):
    ...
    def __init__(self, args):
        ...
        self.source_df = None
        self.target_df = None
        self.accepted_accuracy = float(
            self.args.get("accepted_accuracy", 0.01)
    ...
    def check_sum(self):
        ...
        if is_passed:
            self.logger.info(
                f"Sum check passed: Source sum ({source_sum}) vs Target sum ({target_sum})."
            )
        else:
            message = (
                f"Sum mismatch: Source sum ({source_sum}) != Target sum ({target_sum})."
            )
    ...
    def check_row_quantity(self):
        ...
        if is_passed:
            self.logger.info(
                f"Row quantity check passed: {source_count} rows match {target_count} rows."
            )
        else:
    ...
    def process(self):
        ...
        self.load_data()
        self.logger.info(
            f"Loaded source data from {self.source_pạth} and target data from {self.target_path}."
        )
        ...
        if self.audit_type == "summary":
            ...
            self.send_notification(
                f"{self.JOB_NAME} - Unsupported audit type: {self.audit_type}"
            )
