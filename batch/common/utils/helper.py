...
def generate_date_ranges(
    start_date, end_date, date_format="default", granularity="daily"
):
    if date_format not in DATE_FORMATS:
        raise ValueError(
            f"Invalid date_format '{date_format}'. Supported: {list(DATE_FORMATS.keys())}"
        )

    date_format_str = DATE_FORMATS[date_format]
    print(f"Using date format: {date_format_str}")
    print(
        f"Generating date range from {start_date} to {end_date} with granularity '{granularity}'"
    )
...
def parse_date(value, date_format_str):
    ...
    if isinstance(value, str):
        try:
            ...
            raise ValueError(
                f"Cannot parse '{value}' with format '{date_format_str}': {e}"
            )
