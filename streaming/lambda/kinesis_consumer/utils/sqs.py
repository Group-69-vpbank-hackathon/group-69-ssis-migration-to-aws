...
sqs_client = boto3.client(
    "sqs", config=Config(retries={"max_attempts": 3, "mode": "standard"})
)
...
def send_batch_to_dlq(records_list, dlq_url):
    ...
    try:
        ...
        for table, records in records_by_table.items():
            print(
                f"Preparing to send {len(records)} records for table '{table}' to DLQ"
            )
            ...
            for r in records:
                ...
                if current_size + body_size > MAX_SQS_MESSAGE_SIZE:
                    ...
                    print(
                        f"✅ Sent batch with {len(current_batch)} records for table '{table}'"
                    )
            ...
            if current_batch:
                ...
                print(
                    f"✅ Sent final batch with {len(current_batch)} records for table '{table}'"
                )
